...
DEFAULT_PEER1_MODEL = "smollm2:360m-instruct-q8_0"
...
# -----------------------------------------------------------------------------
@app.post("/generate")
async def generate_and_operations(request: Request):
    """
    JSON body with:
      {
    ...
    try:
        ...
        return JSONResponse({"error": "No JSON body provided."}, status_code=status.HTTP_400_BAD_REQUEST)
    ...
    if not user_prompt:
        return JSONResponse({"error": "No prompt field provided in JSON."}, status_code=status.HTTP_400_BAD_REQUEST)
...
def main():
    # Initialize CSV before running the server
    init_csv()
